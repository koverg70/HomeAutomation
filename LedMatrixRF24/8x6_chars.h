...
int char_cols = 6;	// ennyi pixel oszlop egy karakter

inline uint8_t matrixColBits(uint8_t start, const char *text)
{
	uint8_t disp = text[start / char_cols];
	uint8_t col = start % char_cols;

	return pgm_read_byte(&font[disp][col]);
}
